...
import { resetMockApi } from '../utils/mockApiReset';
import {
  Box,
  Button,
  Container,
...
const Login = () => {
  ...
  // Reset mock API state on component mount (development only)
  useEffect(() => {
    if (process.env.NODE_ENV !== 'production') {
      console.log('Login component mounted');
      
      // Only reset mock API state, don't attempt to login
      resetMockApi();
      
      // Remove the test user login attempt
      // This was causing unnecessary API calls on page load
    }
  }, []);
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      const response = await login(formData).catch(error => {
        console.error('Login page: Login promise rejected:', error);
      ...
      console.error('Login page: Unexpected error in login handler:', error);
  ...
  return (
    <>
      <SEO 
        title="Sign In - LostMyPhone"
        description="Sign in to access your emergency contacts. Use your personal details to access your contacts even when your phone is lost or damaged."
        canonicalUrl="/login"
      />
          ...
          <VStack spacing={8} align="stretch">
            <Box textAlign="center">
              ...
              <Text fontSize="lg" color="gray.600">
                Log in to access your emergency contacts
              </Text>
            </Box>
